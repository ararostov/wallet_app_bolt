// Email-change OTP screen — POST /user/contact/email/verify.
...
export default function EmailChangeOtpScreen() {
  ...
  useEffect(() => {
    if (!inProgress || inProgress.field !== 'email') {
      router.replace('/profile/contact/email');
    }
  }, [inProgress, router]);
    ...
    try {
      ...
      if (e instanceof ApiError) {
        ...
        if (e.code === 'CONTACT_IDENTIFIER_ALREADY_TAKEN') {
          ...
          Alert.alert(
            'Email already taken',
            'This email was claimed while you were verifying. Please choose another.',
          );
          router.replace('/profile/contact/email');
          return;
        }
        if (e.status === 404) {
          dispatch({ type: 'CONTACT_CHANGE/ABORT' });
          Alert.alert('Verification expired', 'Please start the email change again.');
          router.replace('/profile/contact/email');
          return;
        }
        setError(e.message);
  ...
  const handleResend = async () => {
    ...
    try {
      resend.rotateKey();
      await resend.mutate({ newEmail: inProgress.newValue });
  ...
  return (
    <SafeAreaView edges={['left', 'right']} style={[styles.safe, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border, paddingTop: insets.top + 14 }]}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>Verify new email</Text>
        <View style={{ width: 36 }} />
      </View>
